...
- ✂️ 長錄音自動分段轉錄：超過 15 分鐘的音訊自動切成 10 分鐘一段分別轉錄再合併時間戳。實測 53 分鐘會議由 2 軌／70 段改善為 5 軌／451 段
- 👥 可指定與會人數，強化說話者分軌
- 📖 專有名詞校正表：貼上「易錯音→正確寫法」對照，消除廠商名、部門名、術語誤植（常用詞表可存於 Streamlit Secrets 的 `TERM_TABLE`）
